...
if __name__ == "__main__":
	...
	for o in objects:
		...
		frequency, chisq = o.getPgram()
		chisqRange = max(chisq) - min(chisq)
		ytop = min(chisq) + chisqRange / 4
		print "Plotting PGRAM for %s.... %d points in the periodogram"%(o.id, len(frequency))
		LSFrequency = frequency[numpy.argmin(chisq)]
		...
		sampledFrequency, sampledChisq = o.getSampledPgram(n=20)
		# matplotlib.pyplot.plot(frequency, power, linewidth=1.0, color='k', alpha=0.75)
		matplotlib.pyplot.plot(sampledFrequency, sampledChisq, linewidth=1.0, color='k', alpha=0.75)
		matplotlib.pyplot.ylim([0, ytop])
		matplotlib.pyplot.plot([LSFrequency, LSFrequency], [0,  1.2 * max(chisq)], linewidth=1.5, linestyle='--',  color='b', alpha=1)
		matplotlib.pyplot.plot([1/o.ephemeris.Period, 1/o.ephemeris.Period], [0,  1.2 * max(chisq)], linewidth=1.5, linestyle='--',  color='r', alpha=1)
		matplotlib.pyplot.text(0.1, 0.03, "$\chi^2_{red}$ = %3.1f"%o.ephemeris.chi2red, fontsize='x-large', transform = ax2.transAxes)
		matplotlib.pyplot.text(0.7, 0.05, "N = %d"%o.ephemeris.N, fontsize='x-large', transform = ax2.transAxes)

		if plotIndex%2==0: matplotlib.pyplot.ylabel('$\chi^2$')
		matplotlib.pyplot.xlabel('Frequency [cycles/day]')

		zoomFraction = 0.5
		zoomWindow = 2.0 / 2.0   # Size of zoom window in cycles/day
		ax3 = inset_axes(ax2, width="30%", height="50%", loc=1)
		zoomedFrequency = []
		zoomedChisq = []
		bestFrequency = 1.0/o.ephemeris.Period
		for index, f in enumerate(sampledFrequency):
			if (f > (bestFrequency - zoomWindow)) and (f < (bestFrequency + zoomWindow)):
				zoomedFrequency.append(f)
				zoomedChisq.append(sampledChisq[index])

		matplotlib.pyplot.ylim([0, ytop])
		ax3.plot(zoomedFrequency, zoomedChisq, linewidth=1.0, color='k', alpha=1.0)
		ax3.plot([bestFrequency, bestFrequency], [0, max(chisq)], color='r', linewidth=1.5, linestyle='--')
