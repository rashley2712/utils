...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Uses pgplot to plot light curves from Klaus Beuermann''s format CSV file.')
	...
	parser.add_argument('-i', '--invert', action='store_true', help='Invert y-axis')
	parser.add_argument('--paper', type=float, default=10.0, help='Paper size for PGPLOT. Default is 10.0')
	...
	yColumn = columnNames[2]
	yErrors = columnNames[3]
	...
	if hasEphemeris:
		for photometry in allData:
			phases = []
			for bmjd in photometry[xColumn]:
				jd = bmjd + 2400000.5
	...
	for plotDevice in plotDevices:
		...
		for index, photometry in enumerate(allData):
			...
			if not arg.invert:
				...
				ppgplot.pgenv(min(x_values), max(x_values), upperY, lowerY, 0, 0)
			ppgplot.pgslw(7)
	...
	for plotDevice in plotDevices:
		...
		for index, photometry in enumerate(allData):
			...
			if not arg.invert:
				...
				ppgplot.pgenv(phaseLimits[0], phaseLimits[1], upperY, lowerY, 0, 0)
			ppgplot.pgslw(7)
			ppgplot.pgpt(x_values, y_values, 1)
			ppgplot.pgslw(1)
			ppgplot.pgerrb(2, x_values, y_values, y_errors, 0)
